# Feedback #001 -Dror (Earth Sciences)

**Date:** 22 February 2026
**Source:** WhatsApp
**Status:** Collected (pending review)

---

## Feedback Items

### 1. Remove "Multi-Billion" from De-Risking slide
> ואם הפרויקט הוא בסדר גודל קטן יותר? אולי פשוט עדיף להוריד את ה multi billion כדי שיהיה נגיש לכל סוגי המשקיעים.

**Slide:** 4 (De-Risking)
**Action:** Consider removing "multi-billion" to make it accessible to all investor types. Smaller-scale projects should also feel addressed.

---

### 2. Tone down Regulatory Pathways claim
> נראה לי ארוע מסוכן לקחת על עצמינו את מנגנוני הרגולציה בכל מידה. יש כמה מדינות מאוד בעייתיות שבקשיש עובד יותר טוב מחקיקה..

**Slide:** 4 (De-Risking -Regulatory Pathways pillar)
**Action:** Soften the regulatory claim. Avoid committing to handling regulation in all countries -some corridor nations have problematic regulatory environments. Reframe as mapping/understanding rather than managing.

---

### 3. Remove ADNOC case study -sensitive geopolitics
> לגבי הcase study- לא הייתי שמה מדינה ללא קשר פורמאלי לישראל, על אתר של חברה ישראלית.

**Slide:** 5 (DNA / Credibility)
**Action:** Remove the ADNOC & AVEVA case study. Don't reference a country without formal relations with Israel on an Israeli company's site. Find alternative case study or remove entirely.

---

### 4. Add more domains to Force 2 (Corridor X)
> ב- force 2- האם נכון להוסיף עוד תחומים כגון desertification, food security?

**Slide:** 7 (Forces 1 & 2)
**Action:** Consider adding desertification and food security as additional Corridor X domains alongside Energy, Water, Transportation, Risk Management.

---

### 5. Remove Nimrod Sheffer -running for elections
> נמרוד שפר מתמודד בבחירות- לא חושבת שנכון לכתוב את השם שלו.

**Slide:** 10 (Team)
**Action:** Remove Nimrod Sheffer from the team list -he is running for political office and it's inappropriate to list him.

---

### 6. Remove Round Table slide -not enough partners yet
> הייתי מורידה את השקף של the round table. מסכימה לגמרי עם ההגיון אבל כרגע אין מספיק שחקנים.

**Slide:** 11 (Round Table / Network)
**Action:** Remove or hide the Round Table slide. The logic is sound but there aren't enough partners to display yet. Could be premature.

---

### 7. Rephrase Momentum slide title
> במקום we didn't wait for money, הייתי משנה: what we have achieved thus far. גם הייתי מורידה the machine is already running-זה נשמע מוזר.

**Slide:** 12 (Momentum)
**Action:**
- Change title from "We Didn't Wait for Money" → "What We Have Achieved Thus Far"
- Remove subtitle "The machine is already running" -sounds odd

---

### 8. Simplify/hide end section -feels redundant
> לדעתי מ-2026- צריך להיות מוסתר ולשמור את השקף האחרון. (זה פשוט קצת מסורבל וחוזר)

**Slides:** 13-14 (Timeline + Pipeline)
**Action:** Consider hiding/collapsing the 2026 Window and Pipeline slides. Keep only the final closing slide. Current ending feels cluttered and repetitive.

---

### 9. Remove Avital Leibovich from team list
> היערות של אביטל -remove avital from list of people

**Slide:** 10 (Team)
**Action:** Remove Avital Leibovich from the Founders & Strategic Direction team list.

---

## Positive Feedback

> אגב- אוד אהבתי את הצבעים. משדר יוקרה.

**Colors approved** -luxury/premium feel confirmed.

---

## Summary of Changes Requested

| # | Action | Slide | Priority |
|---|--------|-------|----------|
| 1 | Remove "multi-billion" | 4 | Medium |
| 2 | Soften regulatory claim | 4 | Medium |
| 3 | Remove ADNOC case study | 5 | High |
| 4 | Add desertification, food security | 7 | Low |
| 5 | Remove Nimrod Sheffer | 10 | High |
| 6 | Remove/hide Round Table slide | 11 | High |
| 7 | Rephrase momentum title | 12 | Medium |
| 8 | Hide/simplify 2026 + Pipeline | 13-14 | Medium |
| 9 | Remove Avital Leibovich | 10 | High |
